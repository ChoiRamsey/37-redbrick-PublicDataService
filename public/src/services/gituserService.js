const { gituserDao } = require('../models')
...
const githubSignIn = async (code) => {
	...
	let user = await gituserDao.getUserByemail(html_url);
    
    if(!user){
        await gituserDao.gitSignUp(login, html_url, avatar_url, 4);
        user = await gituserDao.getUserByemail(html_url);
    }
